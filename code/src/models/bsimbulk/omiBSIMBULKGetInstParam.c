 ...
 ***************************************************************************
 *  File        : omiBSIMBULKGetInstParam.c
 *  Date        : 2019_0726
...
ParamId omiBSIMBULKGetInstParamIdByName (String aName)
{
  ...
  if(strlen(aName)<=8)
    return omiGetNode(omiBSIMBULKInstHeader, aName);
  else
  {
    strncpy(word,aName,8);
    word[8]='\0';
    return omiGetNode(omiBSIMBULKInstHeader, word);
  }
}

  
/* end of $RCSfile: omiBSIMBULKGetInstParam.c,v $ */
